...
col1.markdown('<h1 style="margin-bottom:0rem;margin-top:-4rem;text-align: center">2D Helmert Transformation</h1>',unsafe_allow_html=True,)
...
with col1:
    ...
    with tab1:
        ...
        In this app the centre of gravity method (and not the least squares method) is used to calculate the transformation parameters. 
        """)

    with tab2:
        st.markdown(
        """
        - **Upload Coordinates:** The user can upload files with the coordinates in the source and target system. The files must have at least three columns with column one for the point name, column 2 for the East coordinate and column 3 for the North coordinate. If nothing is uploaded example coordinates are used.
        - **Select Common Points:** Common points are identified by the point name. 
          ...
          The transformation parameters are re-calculated after every change in the selection of common points.
        - **Download Transformed Points:**  The coordinates that were uploaded as source coordinates are transformed 
          with the calculated transformation parameters.  
