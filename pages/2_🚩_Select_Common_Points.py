...
def deg_to_degminsec(degrees):
    ...
    secint = int(seconds)
    #print(f"{degrees} {degint} {minutes} {minint} {seconds} {secint}")
...
if number_of_cp == 0:
    ...
    chart_points=alt.Chart(st.session_state.common_points_df,title=title1).mark_circle(size=60).encode(
        ...
        #alt.X('y_1', title=st.session_state.source_header_names[1],scale=alt.Scale(zero=False,domain=[st.session_state.common_points_df['y_1'].min(),st.session_state.common_points_df['y_1'].min()+range_all])),
        #alt.Y('x_1', title=st.session_state.source_header_names[2],scale=alt.Scale(zero=False,domain=[st.session_state.common_points_df['x_1'].min(),st.session_state.common_points_df['x_1'].min()+range_all])),
    ...
    with exp:
        ...
        col_e01.markdown(f'<p style="margin-bottom:0rem;margin-top:-0.5rem;text-align: left">Mean:</p>', unsafe_allow_html=True)
        ...
        col_e11.markdown(f'<p style="margin-bottom:-0.5rem;margin-top:0rem;text-align: left">Shift Mean:</p>', unsafe_allow_html=True)
        ...
        with col_e22:
            st.markdown(f'<p style="margin-bottom:0rem;margin-top:0rem;text-align: left">Transformation Parameters:</p>', unsafe_allow_html=True)
